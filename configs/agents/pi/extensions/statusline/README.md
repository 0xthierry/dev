...
- Cloudflare BDR quote from Yahoo Finance (`N2ET34.SA`) in BRL, shown only while the quote is below R$55 by default.
...
| `PI_STATUSLINE_STOCK_MAX_PRICE` | `55` | Hide the stock segment unless the quote is below this price. Set to `off` to always show fetched quotes. |
...
```bash
# Defaults: Cloudflare BDR, BRL, only shown while below R$55
pi
