...
import { z } from "zod";
import type { DirectServiceDependencies } from "../broker/direct-service.ts";
import { DirectSessionExecutor } from "../broker/session-executor.ts";
import { COMPUTER_USE_METHODS, type DirectMethod, type JsonObject, type JsonValue } from "../broker/tools.ts";
...
/** @internal Worker protocol surface used by the fixed production worker and local executor tests. */
export interface CodeWorkerHost {
  on(event: "message", listener: (message: WorkerMessageInput) => void): this;
...
export class ComputerUseCodeExecutor {
  ...
  constructor(
    sessionExecutor: CodeSessionExecutor = new DirectSessionExecutor(),
    codeSliceTimeoutMs = CODE_SLICE_TIMEOUT_MS,
  ) {
    this.sessionExecutor = sessionExecutor;
    this.codeSliceTimeoutMs = codeSliceTimeoutMs;
  }

  protected createWorker(workerData: { code: string; store: Record<string, JsonValue | undefined> }): CodeWorkerHost {
    return new Worker(new URL("./code-worker.ts", import.meta.url), { workerData });
  }
  ...
  private registerScreenshot(image: ImageValue): JsonObject {
    ...
    if (this.screenshots.size > MAX_SCREENSHOT_HANDLES) {
      const oldestScreenshot = this.screenshots.keys().next().value;
      if (oldestScreenshot !== undefined) this.screenshots.delete(oldestScreenshot);
    }
  ...
  execute(code: string, dependencies: DirectServiceDependencies): Promise<ComputerUseCodeResult> {
    return this.runExclusive(async () => {
      ...
      return new Promise<ComputerUseCodeResult>((resolve, reject) => {
        ...
        const worker = this.createWorker({ code, store: this.store });
        let emittedImages = 0;
        ...
        const armTimer = (): void => {
          ...
          if (settled) return;
          timer = setTimeout(
            () => stopWithError(`execution exceeded ${this.codeSliceTimeoutMs}ms between Computer Use calls`),
            this.codeSliceTimeoutMs,
          );
          timer.unref();
        };
            ...
            .then(async () => {
              ...
              if (message.type === "emit_image") {
                ...
                if (emittedImages >= MAX_EMITTED_IMAGES || emittedImageBytes + imageBytes > MAX_EMITTED_IMAGE_BYTES) {
                  stopWithError(
                    `emitted images exceed ${MAX_EMITTED_IMAGES} images or ${MAX_EMITTED_IMAGE_BYTES} bytes`,
                  );
                  return;
                }
                emittedImages += 1;
              ...
              if (calls.length >= MAX_CALLS) {
                worker.postMessage({
                  type: "call_result",
                  id: message.id,
                  error: `Computer Use code exceeded ${MAX_CALLS} calls`,
                });
                return;
              }
              clearTimer();
              ...
              try {
                ...
                const text = response.content
                  ...
                  .join("\n");
                if (response.isError)
                  throw new Error(text.slice(0, 2_000) || "Official Computer Use returned an error");
        ...
        timer = setTimeout(
          () => fail(new Error("Computer Use code worker failed to start")),
