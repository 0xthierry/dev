...
const DIRECT_TOOL_SCHEMAS = {
  ...
  select_text: z
    .object({
      ...
      prefix: z
        .string()
        .describe("Optional text immediately before the target, used to disambiguate repeated matches")
        .optional(),
      selection: z
        .enum(["text", "cursor_before", "cursor_after"])
        .describe("Whether to select the text or place the cursor before or after it. Defaults to text.")
        .optional(),
      suffix: z
        .string()
        .describe("Optional text immediately after the target, used to disambiguate repeated matches")
        .optional(),
    })
    .catchall(additionalArgument),
  scroll: z
    .object({
      ...
      pages: z
        .number()
        .describe("Number of pages to scroll. Fractional values are supported. Defaults to 1")
        .optional(),
    })
    .catchall(additionalArgument),
  drag: z
    ...
    .catchall(additionalArgument),
  press_key: z
    .object({ app, key: z.string().describe("Key or key combination to press") })
    .catchall(additionalArgument),
