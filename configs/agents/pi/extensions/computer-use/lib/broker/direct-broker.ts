...
import { UPSTREAM_VERSION } from "./version.ts";
...
const COMPUTER_USE_CLIENT_PATH = path.join(
  os.userInfo().homedir,
  ".codex",
  COMPUTER_USE_APP_RELATIVE_PATH,
...
function verifySignedBinary(binaryPath: string, runSync: RunSync): void {
  ...
  if (verify.status !== 0)
    throw new BrokerVerificationError(`Signature verification failed for ${path.basename(binaryPath)}`);
...
function checkedCandidate(
  appPath: string,
  clientPath: string,
  layout: OfficialComputerUseClient["layout"],
  runSync: RunSync,
): OfficialComputerUseClient | undefined {
  ...
  if (
    canonicalApp !== path.resolve(appPath) ||
    canonicalClient !== path.resolve(clientPath) ||
    !canonicalClient.startsWith(`${canonicalApp}${path.sep}`)
  ) {
...
export function resolveOfficialComputerUseClient(
  options: ResolveOfficialComputerUseClientOptions = {},
): OfficialComputerUseClient {
  ...
  const current = checkedCandidate(
    currentApp,
    path.join(currentApp, CLIENT_RELATIVE_PATH),
    "installed-component",
    runSync,
  );
  ...
  const legacy = checkedCandidate(
    legacyApp,
    path.join(legacyApp, CLIENT_RELATIVE_PATH),
    "legacy-plugin-bundle",
    runSync,
  );
...
export function verifyOfficialDirectBroker(options: ResolveOfficialComputerUseClientOptions = {}) {
  ...
  const build = runSync("/usr/bin/plutil", [
    "-extract",
    "CFBundleVersion",
    "raw",
    path.join(client.appPath, "Contents", "Info.plist"),
  ]);
...
export function buildDirectAppServerArgs(
  mcpCwd = COMPUTER_USE_PLUGIN_ROOT,
  clientPath = COMPUTER_USE_CLIENT_PATH,
): string[] {
  ...
  const disabledProvider =
    '{ name = "Direct dispatch disabled provider", base_url = "http://127.0.0.1:9/v1", wire_api = "responses", request_max_retries = 0, stream_max_retries = 0, supports_websockets = false, requires_openai_auth = false }';
  return [
    "-c",
    ...
    "-c",
    `model_providers.direct_disabled=${disabledProvider}`,
    "-c",
...
function collectDescendants(rootPid: number, processEnumeratorCommand = "/usr/bin/pgrep"): Set<number> {
  ...
  while (queue.length > 0) {
    ...
    if (
      ...
      throw new ProcessEnumerationError(
        "Could not enumerate the official app-server process tree",
        descendants,
        result.error,
      );
    }
...
function collectProcessesWithCwd(workDir: string): Set<number> {
  ...
  if (
    ...
    throw new ProcessEnumerationError(
      "Could not enumerate processes owned by the private broker working directory",
      new Set(),
      result.error,
    );
  }
...
async function terminateGroup(
  ...
  if (rootWasAlive && !rootFrozen)
    cleanupError ??= new Error("Could not freeze the official app-server before cleanup");
  ...
  for (let pass = 0; pass < 16; pass += 1) {
    ...
    for (const child of enumerate()) {
      ...
      try {
        ...
        if (processExists(child)) cleanupError ??= new Error("Could not freeze an app-server-owned process");
      }
    }
    if (!added && pass > 0) {
      stable = true;
  ...
  try {
    cwdSurvivors = collectProcessesWithCwd(workDir);
  } catch (error) {
    cleanupError ??= error instanceof Error ? error : new Error(String(error));
  }
...
export async function createOfficialDirectToolSession(
  ...
  const pending = new Map<
    string,
    { resolve(value: JsonValue): void; reject(error: Error): void; timer: NodeJS.Timeout }
  >();
  ...
  const close = (): Promise<void> => {
    closePromise ??= (async () => {
      ...
      try {
        ...
        const closedProcess = processClosed;
        if (closedProcess) {
      ...
      if (fatalError || modelTurnsStarted !== 0)
        throw fatalError ?? new BrokerVerificationError("Model-turn activity was observed during broker teardown");
    })();
    return closePromise;
  };
  ...
  try {
    ...
    proc.once("close", (code) => {
      if (!closed && pending.size > 0)
        fail(new Error(`Official app-server exited before completing the request (${code ?? "unknown"})`));
    });

    const processProtocolLine = (line: string): void => {
      let parsedMessage: ReturnType<typeof protocolMessageSchema.safeParse>;
      try {
        parsedMessage = protocolMessageSchema.safeParse(JSON.parse(line));
      } catch {
        fail(new Error("Official app-server emitted malformed JSONL"));
        return;
      }
      if (!parsedMessage.success) {
        fail(new Error("Official app-server emitted an invalid protocol message"));
        return;
      }
      ...
      try {
        if (message.id != null && (message.result !== undefined || message.error !== undefined)) {
          ...
          clearTimeout(waiter.timer);
          if (message.error)
            waiter.reject(new Error(`Official app-server error: ${message.error.message ?? "unknown"}`));
    ...
    await request(
      "initialize",
      {
        clientInfo: { name: "pi_direct_computer_use", title: "Pi Direct Computer Use", version: UPSTREAM_VERSION },
    ...
    throw new DirectBrokerCallError(primary.message, true, primary, {
      modelTurnsStarted,
      ephemeralThread,
  ...
  return {
    async call(method, args, callOptions = {}) {
      ...
      if (callActive)
        throw new DirectBrokerCallError("Official direct Computer Use session does not allow concurrent calls", false);
      callActive = true;
      ...
      try {
        ...
        if (modelTurnsStarted !== 0)
          throw new BrokerVerificationError("Model-turn activity was observed during direct dispatch");
...
export async function callOfficialDirectTool(
  ...
  try {
    ...
    if (closeFailure.message !== "Official direct Computer Use broker cleanup failed") {
      throw new DirectBrokerCallError(
        closeFailure.message,
        true,
        closeFailure,
        failure instanceof DirectBrokerCallError ? failure : {},
      );
    }
    throw new DirectBrokerCallError(
      closeFailure.message,
