...
export function resolveSupportedFile(adapter: LspServerAdapter, root: string, filePath: string): string {
...
function collectPath(
  ...
  if (stats.isFile()) {
    const realFile = realpathSync(targetPath);
    if (adapter.isSupportedFile(targetPath) && !seen.has(realFile)) {
      seen.add(realFile);
...
function resolveWorkspacePath(root: string, inputPath: string, label: string): string {
  ...
  if (existsSync(resolvedPath)) {
    ...
    return isLexicallyInsideRoot ? resolvedPath : path.join(root, path.relative(realRoot, realResolvedPath));
  }
