...
import { collectSupportedFiles, resolveRoot } from "./files";
...
export function selectFixRoute(
  adapters: LspServerAdapter[],
  params: SingleFileRouteParams,
): { root: string; route: SingleFileRoute } {
  const root = resolveRoot(params.root);
...
function unsupportedFileError(action: LspAction, filePath: string, server: string | undefined): Error {
