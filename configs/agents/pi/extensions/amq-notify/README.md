...
Makes Pi main and worker sessions receive AMQ messages through Pi's extension API
without model-side polling or synthetic terminal input.
...
- **Unset `AM_ROOT` (plain `pi`, the main):** it mints a unique session per process,
  ...
  for the next pass instead of silently dropping them. Monitoring and acknowledgement
  use AMQ's strict handle validation.
- **Inherited `AM_ROOT` (a coop-exec worker, e.g. Pi launched by another agent's
  `use-agent`):** it preserves the inherited `AM_ROOT`/`AM_ME`, watches that exact
  worker inbox, and injects tasks through `sendUserMessage`. The Pi worker recipes
  set `AMQ_NOTIFY_ROLE=worker` explicitly so a parent main's role marker cannot leak
  into the child, and use AMQ's `--wake-inject-mode none`, so the companion wake can provide attention
  and lifecycle evidence but can never type into Pi's composer or an approval dialog.
...
- `lib/monitor.ts` — parse `amq monitor --peek --json`, including structured task context, and format notices.
