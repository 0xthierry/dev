...
pi install ./configs/agents/pi/extensions/browser-use
```
...
For day-to-day use on Thierry's machines, `configs/agents/install.sh` symlinks this directory to `~/.pi/agent/extensions`, so no `pi install` command is required. `herdr-agent-state.ts`, `moshi-hooks.ts`, and `ai-memory-pi.ts` are kept byte-for-byte as generated by their pinned releases so vendor freshness checks remain authoritative. Do not run `ai-memory install-hooks --agent pi --apply`; that writes through the symlink and dirties this file. Generated files are excluded from Biome so vendor formatting remains intact. The Moshi and ai-memory files are also excluded from the repository TypeScript project because their generated types differ from Pi's compile-time unions while remaining valid at runtime.
...
- [`browser-use`](./browser-use/README.md) — drives the user's connected Chromium browser through the ChatGPT browser extension and `browser_use`.
