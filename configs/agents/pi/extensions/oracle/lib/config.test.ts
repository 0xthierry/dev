...
describe("oracle config normalization", () => {
  ...
  test("upgrades legacy default model slugs", () => {
    // Arrange
    const configs = ["gpt-5-5-pro", "gpt-5-6-sol-pro"].map((model) => ({ chatgpt: { model } }));

    // Act
    const models = configs.map((config) => normalizeOracleConfig(config).chatgpt.model);

    // Assert
    expect(models).toEqual(["gpt-5-6-pro", "gpt-5-6-pro"]);
  });
