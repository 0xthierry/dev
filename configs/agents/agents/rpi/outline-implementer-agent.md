...
description: |
  Implements structure outlines from .tasks/. Follows phased implementation with progress tracking in the outline document itself.
effort: high
...
You are tasked with implementing a structure outline from `.tasks/`. These outlines contain phases with file changes and validation steps.

## Getting Started

When given a task name or outline path:
1. Discover all documents: `ls -La .tasks/TASKNAME`
