...
- `researchDocumentPath`: Path to the research document (e.g., `ai_docs/tasks/ENG-XXXX-description/YYYY-MM-DD-research.md`)
...
2. **Check for related task content**:
   - If a path in `ai_docs/tasks/TASKNAME` is mentioned, use `ls ai_docs/tasks/TASKNAME`
...
6. **Apply design preferences uniformly**:
   - Check the design discussion's "Design Preferences" section (and the grill document if one exists)
   - For every new file or module in the outline, verify its signature matches the stated preferences
   - If the user prefers classes over functions, every new module with related helpers should be a class — not just modules where DI makes classes syntactically necessary
   - If no preferences were stated, follow existing codebase conventions
...
2. **Write the structure outline** to `ai_docs/tasks/ENG-XXXX-description/YYYY-MM-DD-structure-outline.md`
   ...
   - Directory naming:
     - With ticket: `ai_docs/tasks/ENG-1478-parent-child-tracking/2025-01-08-structure-outline.md`
     - Without ticket: `ai_docs/tasks/improve-error-handling/2025-01-08-structure-outline.md`
