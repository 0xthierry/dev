...
description: >
  ...
  workspace settings, display config, user-facing omarchy commands, and privileged
  Omarchy system configuration using sudo or graphical polkit/pkexec authentication.
  Excludes Omarchy
...
This customized skill is repo-owned at `~/dev/configs/agents/skills/omarchy/`.
The Pi, Claude, Codex, and `.agents` skill locations symlink here. Edit this
canonical copy, not Omarchy's upstream copy. The shared agent installer discovers
it automatically. Upstream skill changes must be reviewed and merged explicitly.
...
## Privileged Commands: Graphical Authentication with pkexec

**A password-required `sudo` failure is not necessarily a blocker on an active
Omarchy desktop.** `pkexec` can request administrator authorization through the
session's graphical polkit authentication agent. The user enters their password
in the trusted desktop dialog; the model never sees or handles it. The authorized
command's output and exit status return to the normal command tool. This works
with any agent harness capable of executing commands in that desktop session.

### When and how to use it

1. **Stay within the user's authorized task.** Use ordinary unprivileged commands
   first. If a necessary command needs root, try `sudo -n /absolute/program args`
   to avoid a hanging terminal password prompt. Only consider this fallback when
   interactive authentication is needed—not to evade an explicit authorization
   denial, sandbox restriction, or tool policy.
2. **Check availability without prompting:**
   ```bash
   command -v pkexec
   ps -eo comm | grep -E 'polkit|hyprpolkit'
   ```
   Omarchy may run `polkit-gnome-authentication-agent-1` (its process name can be
   truncated), `hyprpolkitagent`, or another agent. `polkitd` alone does not prove
   that a graphical authentication agent is registered. An SSH/headless session
   may not have access to the desktop agent even when the process exists.
3. **Tell the user before opening the dialog:** explain the exact operation,
   affected files, and relevant risks. For example: “I'll request graphical
   authorization to read the filesystem error counters. Enter your password in
   the desktop dialog, not in chat.” Keep changes within the agreed scope.
4. **Run one bounded command**, allowing time for the user to approve it:
   ```bash
   pkexec --disable-internal-agent /usr/bin/btrfs device stats /
   ```
   Set the command tool's timeout to approximately **120–180 seconds** for a short
   operation with interactive approval. `--disable-internal-agent` prevents
   fallback to pkexec's textual password agent when no session agent is available.
   Polkit policy determines whether authentication is required; do not promise
   that a fresh dialog will appear every time.
5. **Verify the actual result**, including configuration validation and live
   state where relevant. Approval alone is not proof that the operation succeeded.

### Multi-step system changes

Prefer invoking a specific executable directly. When a change genuinely needs
multiple root-only steps, prepare a **small, single-purpose script** first:

- Keep it in a private, user-owned directory; do not use a predictable shared
  `/tmp` path. Follow repo ownership rules for persistent setup/configuration.
- Review its complete contents, run `bash -n`, and run ShellCheck if available.
- Use `set -Eeuo pipefail`, a known system `PATH`, explicit absolute paths, and
  precondition checks. Back up configuration before changes. Refuse unexpected
  existing files instead of overwriting them; design safe retry/recovery behavior.
- Explain what the script will do. The authentication dialog may primarily name
  the interpreter, so it is not a substitute for reviewing the script's scope.
- Do not modify the script while approval or execution is pending. Execute only
  that reviewed script, not a general-purpose interactive root shell:
  ```bash
  pkexec --disable-internal-agent /usr/bin/bash /absolute/path/to/reviewed-script
  ```
- `pkexec` sanitizes the environment and normally changes the working directory
  to the target user's home. Do not rely on the caller's `HOME`, working directory,
  shell functions, mise shims, or relative paths. Do not forward the entire
  environment or weaken polkit policy to work around this.

### Cancellation, failures, and safety

- Never request a password in chat, pipe one to `sudo -S`, record it, automate
  typing into the authorization dialog, or inspect/capture that dialog.
- Do not add passwordless sudo rules or permissive polkit rules for convenience.
- If the user cancels, stop; do not repeatedly prompt. pkexec normally returns
  **126** for dismissal and **127** for an authorization/error failure; a launched
  command can also return those codes, so inspect output and actual state.
- On a tool timeout, inspect whether execution is still running and whether
  changes occurred before retrying. A timeout is not proof of rollback or failure
  to start. Avoid timeouts that could interrupt long-running critical operations.
- If graphical authentication is unavailable or denied, report the blocker and
  give the user the exact narrow `sudo` command to run in their own terminal.
- Continue using `omarchy debug --no-sudo --print`; this workflow is for separately
  justified privileged operations, not for elevating the entire debug collection.

Reference: local `man pkexec` (authentication agent, environment, and exit status).
This workflow was verified on this workstation when enabling SSD-backed swap.
