...
1. **Read all mentioned files immediately and FULLY**:
   - Ticket files (e.g., `ai_docs/tasks/eng-1234-description/ticket.md`)
   - Research documents (e.g. `ai_docs/tasks/eng-1234-description/research.md`)
   - **IMPORTANT**: Use the Read tool WITHOUT limit/offset parameters to read entire files
   - **CRITICAL**: DO NOT spawn sub-tasks before reading these files yourself in the main context
   - **NEVER** read files partially - if a file is mentioned, read it completely

2. **Check for related task content**:
   - if a path in `ai_docs/tasks/TASKNAME` is mentioned, use Bash(`ls ai_docs/tasks/TASKNAME`)
   - read all relevant files in the task directory to fully understand the work so far
   - check `ai_docs/grills/` for related grill documents — if one exists, carry forward any resolved decisions and design preferences
...
3. **Discuss design preferences** before finalizing the document
   - Ask the user about cross-cutting style constraints that should apply to all new code: code organization (classes vs functions), module cohesion, naming, error handling approach, etc.
   - Also watch for preferences the user states organically during the discussion (e.g. "I prefer classes for grouping related logic") — capture these in the Design Preferences section even if the user didn't frame them as a formal preference
   - These are different from design decisions: decisions are per-feature ("use ts-morph for inference"), preferences are cross-cutting ("all new modules should be classes")
   - Populate the "Design Preferences" section in the document — this section is carried forward into the structure outline and plan, where it governs how every new module is written
...
2. **Write the design discussion** to `ai_docs/tasks/ENG-XXXX-description/YYYY-MM-DD-design-discussion.md`
   ...
   - Directory naming:
     - With ticket: `ai_docs/tasks/ENG-1478-parent-child-tracking/2025-01-08-design-discussion.md`
     - Without ticket: `ai_docs/tasks/improve-error-handling/2025-01-08-design-discussion.md`
