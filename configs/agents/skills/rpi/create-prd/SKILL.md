...
<instructions>
- Find the task directory: `ls -La .tasks/TASKNAME` (use Bash `ls -La` - not Grep/Glob - the directory may be a symlink).
...
<instructions>
Write the doc to `.tasks/ENG-XXXX-description/NN-prd-DESCRIPTION.md` (next zero-padded `NN-` index in the task dir; `DESCRIPTION` is a 2-4 word kebab-case slug).
...
- Write HTML files to `.tasks/{task-slug}/mockup-{description}.html`
