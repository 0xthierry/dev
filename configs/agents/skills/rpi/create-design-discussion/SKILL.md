...
1. **Read the files in the task artifact directory and any mentioned files immediately and FULLY**:
   - `ls -La .tasks/TASKSLUG` to find all related documents in the task directory. Do NOT use the Grep or Glob tools, or `ls -l` as the directory may be a symlink.\
   - Ticket files (e.g., `.tasks/eng-1234-description/ticket.md`)
   - Research documents (e.g. `.tasks/eng-1234-description/NN-research-DESCRIPTION.md`)
...
2. **Check for related task content**:
   - if a path in `.tasks/TASKNAME` is mentioned, use Bash(`ls -La .tasks/TASKNAME`)
   - **IMPORTANT** DO NOT USE search or glob or grep, or `ls -l` (lowercase L), as .tasks may be a symlink and those tools don't traverse symlinks
...
5. **Write the design discussion** to `.tasks/ENG-XXXX-description/NN-design-discussion-DESCRIPTION.md`
   - First, check the task directory in your system prompt. If you don't see it, find the task directory: `ls -La .tasks | grep -i "eng-XXXX"`
   - If the directory doesn't exist, create: `.tasks/ENG-XXXX-description/`
   ...
   - Directory naming:
     - With ticket: `.tasks/ENG-1478-parent-child-tracking/03-design-discussion-parent-child-tracking.md`
     - Without ticket: `.tasks/improve-error-handling/03-design-discussion-error-handling.md`
...
7. Respond following the selected template exactly. Do not include a summary or other information. Include artifact paths if available.
